    ...
    <div className="space-y-2">
      <div className="flex items-center gap-3">
        <div className="relative">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--fg-dim)]" />
          <input
            type="text"
            placeholder="Search videos..."
            value={view.search}
            onChange={(e) => viewStore.setSearch(e.target.value)}
            className="h-7 pl-8 pr-3 w-56 rounded-md border bg-[var(--bg-raised)] text-xs outline-none"
          />
        </div>

        {!hasFilters && (
          <>
            <div className="h-4 w-px bg-[var(--line-faint)]" />
      ...
      {hasFilters && (
        <div className="flex items-center gap-2 pt-1 border-t border-[var(--line-faint)]">
          <FilterChipRow videos={videos} />
        </div>
      )}
    </div>
